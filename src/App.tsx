...
const PRESETS = [
  {
    id: "p_nature",
    name: "妈生感自然微调",
    projects: { n_bridge: 1, e_double: 1, t_face: 1 },
  },
  {
    id: "p_celeb",
    name: "高清上镜精致脸",
    projects: { n_bridge: 3, n_wing: 3, c_jaw: 4, t_face: 3 },
  },
  {
    id: "p_light",
    name: "轻医美年轻化",
    projects: { e_tear: 4, f_naso: 3, e_bags: 4 },
  },
];
...
const RECOVERY_TIMELINE = [
  ...
  },
  {
    ...
    label: "7d",
    stage: "快速消肿期",
    desc: "拆线，伤口结痂，淤青转黄（血铁黄素沉积）",
  },
  {
    val: 14,
    label: "14d",
    stage: "持续消肿期",
    desc: "肿胀消退80%，轮廓清晰，进入组织修复",
  },
  {
    val: 30,
    label: "1m",
    stage: "组织恢复期",
    desc: "疤痕增生初期，线条渐趋自然，仍有微肿",
  },
  {
    val: 90,
    label: "3m",
    stage: "最终成型期",
    desc: "疤痕软化，形态稳定，获得最佳临床效果",
  },
  {
    val: 365,
    label: "1y",
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-10 text-center">
          <div className="w-20 h-20 bg-rose-500/10 rounded-full flex items-center justify-center mb-8">
            <AlertTriangle className="w-10 h-10 text-rose-500" />
          </div>
          <h1 className="text-white font-black text-3xl mb-4 tracking-tighter">
            Oops! App Encountered an Error
          </h1>
          <p className="text-slate-400 text-lg mb-8 max-w-md mx-auto leading-relaxed">
            {this.state.error?.message || "Something went wrong during rendering."}
          </p>
          <button
            onClick={() => {
               localStorage.clear();
            ...
            className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20"
          >
            Reset App & Data
          </button>
          <p className="mt-6 text-slate-600 text-xs uppercase tracking-[0.3em] font-bold">
...
export default function App() {
  const [beautyUploads, setBeautyUploads] = useState<{
    before: string | null;
    after: string | null;
  }>({
    before: null,
  ...
  const [beautyResults, setBeautyResults] = useState<
    Array<{
      id: string;
      url: string;
      createdAt: number;
  ...
  >(() => {
    try {
      const saved = localStorage.getItem("beauty_material_library");
      const parsed = saved ? JSON.parse(saved) : [];
      return Array.isArray(parsed) ? parsed : [];
    } catch (e) {
      console.error("Failed to load beautyResults", e);
      return [];
    }
  });
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem(
        "beauty_material_library",
        JSON.stringify(beautyResults),
      );
    } catch (e) {
      console.warn("localStorage quota exceeded for beauty_material_library", e);
    }
  }, [beautyResults]);
  ...
  // Helper to fetch API Key from multiple sources
  const getApiKey = () => {
  ...
  const handleBeautyGen = async () => {
    setIsGeneratingVlog(true);
    setVlogGenProgress(0);
    setVlogGenStatus("正在构思医美素材视觉方案...");
    try {
      const apiKey = getApiKey();
      if (!apiKey) throw new Error("API Key required");
      const ai = new GoogleGenerativeAI(apiKey);
      const model = ai.getGenerativeModel({ model: "gemini-1.5-flash" });
      const prompt = `你现在是【高端医美视觉设计总监】。生成医美项目视觉海报方案。风格：${faceSwapPrompt}，任务：${faceSwapTaskName}。请 JSON 输出: { "prompt": "AI提示词", "slogan": "文案" }`;
      const parts = [{ text: prompt }];
      if (beautyUploads.before) {
        parts.push({
          inlineData: {
            data: beautyUploads.before.split(",")[1],
            mimeType: "image/jpeg",
          },
        } as any);
      }
      ...
      const result = jsonBlock
        ...
        : { prompt: faceSwapPrompt, slogan: "精致容颜" };
      setVlogGenStatus("正在制作精美素材...");
      setVlogGenProgress(50);
      const seed = Math.floor(Math.random() * 10000);
      const finalUrl = `https://images.unsplash.com/photo-1551076805-e1869033e561?auto=format&fit=crop&q=80&w=800&h=1200&sig=${seed}`;
      setBeautyResults((prev) => [
        {
          id: Math.random().toString(36).substring(7),
          url: finalUrl,
          createdAt: Date.now(),
          taskName: faceSwapTaskName,
          type: "beauty-material",
          source: beautyUploads.before || undefined,
        },
        ...prev,
      ]);
      setVlogGenProgress(100);
      setVlogGenStatus("素材生产完成！");
    } catch (e) {
      console.error(e);
      setVlogGenStatus("生成失败");
    } finally {
  ...
  const [vlogReferenceImage, setVlogReferenceImage] = useState<string | null>(
    null,
  );
  const [vlogCharacterConsistency, setVlogCharacterConsistency] =
    useState(true);
  ...
  const downloadImage = async (url: string, prefixOrFilename = "doc") => {
    ...
    try {
      ...
      link.download = isFullFilename
        ? prefixOrFilename
        : `${prefixOrFilename}-${Date.now()}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      if (isBlob) {
        // Small timeout to ensure browser started the download
  ...
  const downloadBatchAsZip = async (results: any[], filename = "batch_export.zip") => {
    ...
    try {
      ...
      const downloadTasks = results.map(async (res, i) => {
        try {
          const url = res.url;
          let blob;
          if (url.startsWith("data:") || url.startsWith("blob:")) {
          ...
          const name = `image_${i + 1}_${res.id || Date.now()}.${extension}`;
      ...
      alert("批量压缩下载失败，请尝试单个下载。");
    } finally {
      setDownloadingZip(false);
    }
  };
  const [activeTab, setActiveTab] = useState<
    "comparison" | "timeline" | "report" | "multiview"
  >("comparison");
  ...
  const [mainModule, setMainModule] = useState<
    "vlog" | "faceswap" | "simulation" | "batch-image" | "beauty-material"
  >("vlog");
  const [batchImageResults, setBatchImageResults] = useState<
    Array<{
      id: string;
      url: string;
      prompt: string;
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem(
        "batch_image_library",
        JSON.stringify(batchImageResults),
      );
    } catch (e) {
      console.warn("localStorage quota exceeded for batch_image_library", e);
    }
  }, [batchImageResults]);

  const [batchImagePrompts, setBatchImagePrompts] = useState<string[]>(
    Array(10).fill(""),
  );
  const [batchImageTaskName, setBatchImageTaskName] = useState(
    `批量生图_${new Date().toLocaleTimeString("zh-CN", { hour12: false })}`,
  );
  ...
  const [batchImageAspectRatio, setBatchImageAspectRatio] = useState<
    "1:1" | "9:16" | "16:9"
  >("1:1");

  const [faceSwapResults, setFaceSwapResults] = useState<
    Array<{
      id: string;
      url: string;
      createdAt: number;
  ...
  React.useEffect(() => {
    try {
      ...
      console.warn("localStorage quota exceeded for face_swap_library", e);
    }
  }, [faceSwapResults]);
  ...
  React.useEffect(() => {
    try {
      ...
      console.warn("localStorage quota exceeded for simulation_library", e);
    }
  }, [simulationResults]);

  const [faceSwapInputs, setFaceSwapInputs] = useState<{
    source: string | null;
    targets: string[];
  }>({
    source: null,
  ...
  const [faceSwapPrompt, setFaceSwapPrompt] = useState(
    "A cinematic portrait of a person, highly detailed, 8k resolution, photorealistic style, wearing professional attire.",
  );
  const [faceSwapBaseUrl, setFaceSwapBaseUrl] = useState(
    () =>
      localStorage.getItem("face_swap_base_url") ||
      "https://api.img.dengche.cc/v1",
  );
  const [faceSwapApiKey, setFaceSwapApiKey] = useState(
    () => localStorage.getItem("face_swap_api_key") || "",
  );
  const [faceSwapModel, setFaceSwapModel] = useState(
    () => localStorage.getItem("face_swap_model") || "gpt-4o",
  );
  const [faceSwapImageModel, setFaceSwapImageModel] = useState(
    () => localStorage.getItem("face_swap_image_model") || "gpt-image-2",
  );
  const [useGeminiSim, setUseGeminiSim] = useState(() => {
    const saved = localStorage.getItem("use_gemini_sim");
    return saved === null ? true : saved === "true";
  });
  const [faceSwapApis, setFaceSwapApis] = useState<string[]>(
    Array(10).fill("v1/images/edits"),
  );
  const [showApiConfig, setShowApiConfig] = useState(false);
  const [faceSwapTaskName, setFaceSwapTaskName] = useState(
    `任务_${new Date().toLocaleTimeString("zh-CN", { hour12: false })}`,
  );
  const [simulationTaskName, setSimulationTaskName] = useState(
    `模拟_${new Date().toLocaleTimeString("zh-CN", { hour12: false })}`,
  );
  ...
  const [openaiKey, setOpenaiKey] = useState(
    () => localStorage.getItem("vlog_openai_key") || "",
  );
  const [openaiBaseUrl, setOpenaiBaseUrl] = useState(
    () =>
      localStorage.getItem("vlog_openai_base_url") ||
  ...
  React.useEffect(() => {
    ...
    if (
      faceSwapBaseUrl.includes("dengche.cc") &&
      !faceSwapBaseUrl.includes("/v1")
    ) {
      const normalized = faceSwapBaseUrl.endsWith("/")
        ? `${faceSwapBaseUrl}v1`
        : `${faceSwapBaseUrl}/v1`;
  ...
  const [vlogResults, setVlogResults] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem("vlog_library");
      const parsed = saved ? JSON.parse(saved) : [];
      return Array.isArray(parsed) ? parsed : [];
    } catch (e) {
      console.error("Failed to load vlog_library", e);
      return [];
    }
  });
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem("vlog_library", JSON.stringify(vlogResults));
    } catch (e) {
      console.warn("localStorage quota exceeded for vlog_library", e);
    }
  }, [vlogResults]);
  ...
  const [activeVlogTab, setActiveVlogTab] = useState<
    "topics" | "script" | "storyboard"
  >("topics");

  // Vlog Lab Logic
  const handleGenerateVlog = async (
    type: "topics" | "script" | "storyboard" | "tags" | "all",
  ) => {
    setIsGeneratingVlog(true);
    setVlogResult(null);
    setVlogGenProgress(0);
...
TASK: Generate Topics\nTemplate: ${VLOG_TEMPLATES.topics
      ...
      const responseRecord = await model.generateContent({
        contents: [
          {
            role: "user",
            parts: [
      ...
      const getSection = (searchKey: string) => {
        const found = sections.find((s) =>
          s.toUpperCase().includes(searchKey.toUpperCase()),
        );
        if (!found) return "";
        return found.split("\n").slice(1).join("\n").trim();
      };
      ...
      if (type === "all" || type === "topics")
        newResult.topics = getSection("Generate Topics");
      if (type === "all" || type === "script")
        newResult.script = getSection("Generate Script");
      if (type === "all" || type === "tags")
        newResult.tags = getSection("Generate Tags");

      if (type === "all" || type === "storyboard") {
        const fullContent = getSection("Generate Storyboard JSON");
        const jsonBlock = fullContent?.match(/\[[\s\S]*\]/)?.[0];
        if (jsonBlock) {
          try {
            const parsedStoryboard = JSON.parse(jsonBlock);
            if (Array.isArray(parsedStoryboard)) {
              newResult.storyboard = parsedStoryboard;
              newResult.storyboardImages = new Array(
                parsedStoryboard.length,
              ).fill(null);
                ...
                for (let i = 0; i < parsedStoryboard.length; i++) {
                  try {
                    ...
                    if (vlogVars.imageEngine === "dalle") {
                      setVlogGenStatus(
                        `渲染分镜 #${i + 1}/${parsedStoryboard.length}...`,
                      );
                      const response = await fetch("/api/generate-image", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          prompt: clip["Imagen Prompt"] || clip.Scene || "Aesthetic clinic",
                      ...
                      const updateStates = (current: any) => {
                      ...
                      setVlogResult(updateStates);
                      // Also update the latest item in history
                      setVlogResults((prevList) => {
                        if (prevList.length === 0) return prevList;
                        const newList = [...prevList];
                        const lastItem = { ...newList[0] };
                        const newImages = [...(lastItem.storyboardImages || [])];
                        newImages[i] = imageUrl;
                        newList[0] = { ...lastItem, storyboardImages: newImages };
                        return newList;
                      });
                    } else {
                      setVlogGenStatus(`准备模拟素材 #${i + 1}...`);
                    ...
                    if (
                      vlogVars.imageEngine === "dalle" &&
                      e instanceof Error &&
                      e.message !== "Stock simulation"
                    ) {
            ...
            console.error("Storyboard processing error", e);
          }
        } else {
          setVlogGenProgress(100);
        }
      ...
      setVlogResult((prev) => {
        const final = { ...prev, ...newResult };
        // Save to history if we have something meaningful
        if (final.topics || final.script || final.storyboard) {
          setVlogResults((prevList) => [
            {
              id: `${Date.now()}`,
              createdAt: Date.now(),
    ...
    try {
      ...
      if (url.startsWith("data:")) {
        try {
          ...
          console.error(
            `Failed to convert data URL to file: ${filename}`,
            dataErr,
          );
          return null;
        }
  ...
  const handleBatchImageGen = async () => {
    ...
    try {
      const activePrompts = batchImagePrompts
        .map((p, i) => ({ p, i }))
        .filter((x) => x.p.trim() !== "");
      console.log(
        `Starting batch generation of ${activePrompts.length} tasks...`,
      );

      const generationTasks = activePrompts.map(
        ({ p: prompt, i: index }) => async () => {
            ...
            const response = await fetch("/api/face-swap", {
              ...
              body: JSON.stringify({
                ...
                payload: {
                  ...
                  n: 1,
                  size:
                    batchImageAspectRatio === "9:16"
                      ? "1024x1792"
                      : batchImageAspectRatio === "16:9"
                        ? "1792x1024"
                        : "1024x1024",
                  response_format: "url",
                },
      ...
      // Execute with a concurrency limit of 3
      const limit = 3;
      const results = [];
      for (let i = 0; i < generationTasks.length; i += limit) {
        const chunk = generationTasks.slice(i, i + limit);
  ...
  const handleFaceSwap = async () => {
    ...
    const effectiveApis =
      activeApis.length > 0 ? activeApis : ["v1/images/edits"];
    ...
    try {
      ...
      console.log(
        `Starting synthesis of ${total} independent tasks for project [${faceSwapTaskName}]...`,
      );

      const synthesisTasks = faceSwapInputs.targets.map(
        (targetUrl, index) => async () => {
          ...
          let cleanApiPath = apiPath.startsWith("/")
            ? apiPath.slice(1)
            : apiPath;
          if (baseUrl.endsWith("/v1") && cleanApiPath.startsWith("v1/")) {
            cleanApiPath = cleanApiPath.slice(3);
          ...
          try {
            ...
            if (isReferenceMode) {
              ...
              const sourceFile = await getFileFromUrl(
              ...
              if (sourceFile) {
                ...
                console.warn(
                  `[Task ${index}] Source image download failed, sending URL instead if possible`,
                );
              }

              // 2. Target Scene (Base)
              const targetFile = await getFileFromUrl(
                targetUrl,
                `scene_${index}.png`,
              );
              if (targetFile) {
              ...
              console.log(
                `[Task ${index}] Fetching /api/face-swap-multipart...`,
              );
              try {
                ...
                console.error(
                  `[Task ${index}] Network error in multipart fetch:`,
                  netErr,
                );
                throw new Error(
                  `网络请求失败 (Multipart): ${netErr instanceof Error ? netErr.message : "连接异常"}`,
                );
              }
            } else {
              ...
              try {
                ...
                console.error(
                  `[Task ${index}] Network error in json fetch:`,
                  netErr,
                );
                throw new Error(
                  `网络请求失败 (JSON): ${netErr instanceof Error ? netErr.message : "连接异常"}`,
                );
              }
            }
              ...
              try {
                ...
                if (contentType && contentType.includes("application/json")) {
                  ...
                  errorMsg = errorText
                    ...
                    : `HTTP ${response.status} (No response body)`;
                }
              } catch (e) {
            ...
            try {
              ...
              throw new Error(
                `API 返回了非 JSON 格式内容: ${rawText.slice(0, 50)}...`,
              );
            }
            ...
            alert(
              `任务 ${index + 1} 失败: ${apiErr instanceof Error ? apiErr.message : "未知错误"}`,
            );
            return null;
          }
        },
      ...
      // Execute with a concurrency limit of 2 for FaceSwap (more memory intensive)
      const limit = 2;
      const results = [];
      for (let i = 0; i < synthesisTasks.length; i += limit) {
        const chunk = synthesisTasks.slice(i, i + limit);
        results.push(...(await Promise.all(chunk.map((task) => task()))));
      }
  ...
  const handleSaveAsset = async (
    url: string,
    prompt: string,
    type: "vlog_clip" | "face_simulation",
  ) => {
    if (!user) {
      alert("请先登录");
      return;
  ...
  const getBaseSimulationPrompt = () => {
    try {
      ...
      const currentStage =
        RECOVERY_TIMELINE.length > 0
          ? RECOVERY_TIMELINE.reduce((prev, curr) =>
              postOpDays >= curr.val ? curr : prev,
            )
          : { stage: "Unknown", desc: "" };

      const ptPhysique =
        PHYSIQUE_TYPES.find((p) => p.id === physique)?.name || "正常体质";
...
1. Output Requirement: You MUST return both a modified image reflecting the results AND a clinical report in markdown.`;
  ...
  const handleSimulate = async () => {
    if (
      !image ||
      Object.keys(intensities).filter((k) => (intensities[k] || 0) > 0)
        .length === 0
    )
      return;
    ...
    try {
      const activeProjects = Object.keys(intensities).filter(
        (k) => (intensities[k] || 0) > 0,
      );
      const projectSummary = activeProjects
        .map((p) => `${p}(Intensity: ${intensities[p]}%)`)
        .join(", ");
      ...
      if (
        !cleanBaseUrl.includes("/chat/completions") &&
     ...
     `;
      ...
      if (useGeminiSim) {
        try {
          const apiKey = getApiKey();
          if (!apiKey) throw new Error("Gemini API Key 缺失，请检查环境变量");
          const ai = new GoogleGenerativeAI(apiKey);
          const model = ai.getGenerativeModel({ model: "gemini-1.5-flash" });
        ...
        llmResultText = result.response.text() || "";
          if (!llmResultText)
        ...
        if (!llmResponse.ok) {
          ...
          throw new Error(
            errorDetail || `LLM Request failed: ${llmResponse.status}`,
          );
        }
      ...
      const promptMatch = llmResultText.match(
        /<image_prompt>([\s\S]*?)<\/image_prompt>/,
      );

      let reportText = reportMatch ? reportMatch[1].trim() : "";
      let dallEPrompt = promptMatch ? promptMatch[1].trim() : "";

      // Fallback parsing if tags are missing or Gemini returned different format
      if (!reportText) {
        if (llmResultText.includes("###")) {
          reportText = llmResultText.split(/image_prompt/i)[0].trim();
        } else {
          reportText = llmResultText;
        }
      }
      if (!dallEPrompt && !promptMatch) {
         const promptLine = llmResultText.match(/(prompt|提示词)[:：]\s*(.*)/i);
         if (promptLine) dallEPrompt = promptLine[2].trim();
      }
  ...
  const getDayDescription = (days: number) => {
    ...
    return RECOVERY_TIMELINE.reduce((prev, curr) =>
      days >= curr.val ? curr : prev,
    ).desc;
  };
  ...
  const getRecoveryStyle = (day: number) => {
    ...
    if (day === 0) {
      ...
      filter =
        "blur(0.8px) saturate(1.3) hue-rotate(-8deg) contrast(1.15) sepia(0.1)";
    ...
    <ErrorBoundary>
      ...
      <aside className="w-[340px] flex flex-col bg-white border-r border-slate-200 h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
          ...
          <div className="bg-[#F8FAFC] border border-[#F1F5F9] rounded-2xl p-4 space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Monitor className="w-2.5 h-2.5" /> 自动生成提示词 (Base
                Protocol)
              </label>
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <MessageSquare className="w-2.5 h-2.5" /> 自定义专家指令 (Extra
                Prompt)
              </label>
              <textarea
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100">
              ...
              <p className="text-[10px] text-slate-400 italic">
                当前状态: {getDayDescription(postOpDays)}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 rounded-lg px-2 py-1.5 text-[11px] font-bold outline-none focus:ring-1 focus:ring-teal-100"
                >
            ...
            {CATEGORIES.filter((c) => c.dept === selectedDept).map((cat) => (
              ...
              >
                <button
                  ...
                  className="w-full px-4 py-3 bg-[#F8FAFC] flex items-center justify-between text-[11px] font-bold text-slate-600"
                >
                  <span>{cat.name}</span>
                  <ChevronRight
                    className={cn(
                    ...
                    {cat.items.map((item) => (
                      <div key={item.id} className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateIntensity(item.id, parseInt(e.target.value))
                          }
          ...
          <div className="bg-amber-50/50 border border-amber-100 rounded-2xl p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <select
                  value={surgeryMethod}
                  onChange={(e) => setSurgeryMethod(e.target.value)}
            ...
            <div className="grid grid-cols-2 gap-2">
              {PRESETS.map((p) => (
                <button
                  ...
                  className="px-3 py-2 text-[10px] font-bold text-slate-500 bg-white border border-slate-200 rounded-xl hover:border-teal-500 hover:text-teal-600 transition-all text-center truncate"
                >
                  {p.name}
                </button>
              ))}
            ...
            {showApiConfig && (
              <div className="mt-3 space-y-4 p-3 bg-slate-50/50 rounded-xl border border-slate-100 animate-in slide-in-from-top-2 duration-300">
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-2 bg-white border border-slate-100 rounded-lg">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        useGeminiSim ? "bg-teal-500" : "bg-slate-200",
                      )}
                    >
                      <div
                        className={cn(
                          "absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all",
                          useGeminiSim ? "left-4.5" : "left-0.5",
                        )}
                      />
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                      <ImageIcon className="w-2.5 h-2.5" /> 生图引擎模型 (Image
                      Model)
                    </label>
                    <input
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-[60px] bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-6">
            <div className="flex gap-4 text-xs font-bold text-slate-400">
              {["comparison", "multiview", "timeline", "report", "history"].map(
                (tab) => (
                  <button
                    key={tab}
        ...
        <section className="flex-1 p-8 overflow-hidden flex flex-col bg-slate-50">
          {!image ? (
            <div
              {...getRootProps()}
              className="w-full h-full border-2 border-dashed border-slate-200 rounded-[3rem] bg-white flex flex-col items-center justify-center p-12 text-center group cursor-pointer hover:bg-slate-50/50 transition-all duration-500"
            >
              <input {...getInputProps()} />
              ...
              <p className="text-sm text-slate-400 max-w-sm mb-12">
                系统将自动分析 106 个解剖关键点，并基于临床大数据生成仿真。支持
                JPG, PNG 高清格式。
              </p>
              <div className="flex gap-4">
                <span className="px-5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold text-slate-400">
                  等效焦距 50mm+ 优化
                </span>
                <span className="px-5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold text-slate-400">
                  符合 HIPAA/GDPR 规范
                  ...
                  {activeTab === "comparison" ? (
                    ...
                    >
                      <div className="flex-1 relative border-r border-[#1E293B]">
                        ...
                        />
                        <label className="absolute top-6 left-6 bg-black/60 px-4 py-2 rounded-lg text-[10px] font-bold text-white uppercase tracking-[0.2em] backdrop-blur-md border border-white/10">
                          Pre-OP / 术前
                        </label>
                      </div>
                        ...
                        {simulatedImage ? (
                          <React.Fragment>
                            <img
                              ...
                              referrerPolicy="no-referrer"
                            />
                            <label className="absolute top-6 right-6 bg-teal-600/90 px-4 py-2 rounded-lg text-[10px] font-bold text-white uppercase tracking-[0.2em] backdrop-blur-md border border-teal-400/20 shadow-lg">
                              Simulation / 仿真
                            </label>
                            <div className="absolute bottom-6 right-6 bg-black/60 px-4 py-2 rounded-lg text-[10px] font-bold text-teal-400 lowercase italic border border-teal-900/30">
                              Day {postOpDays} Status
                            </div>
                          </React.Fragment>
                        ) : (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              showMatrixTiles ? "bg-teal-600" : "bg-slate-800",
                            )}
                          >
                            <div
                              className={cn(
                                "absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-300",
                                showMatrixTiles ? "right-1" : "left-1",
                              )}
                            />
                          </button>
                        </div>
                        ...
                        <div className="flex-1 grid grid-cols-3 grid-rows-2 p-6 gap-6 overflow-y-auto custom-scrollbar">
                          <div className="relative rounded-2xl overflow-hidden bg-slate-900 border border-[#1E293B]">
                            <img
                              ...
                              className="w-full h-full object-cover"
                            />
                            <div className="absolute top-4 left-4 bg-black/60 px-2 py-1 rounded text-[9px] font-bold text-white uppercase">
                              Baseline / 术前
                            </div>
                          </div>
                          {[3, 7, 30, 90, 365].map((day) => (
                            <div
                              key={day}
                              className="relative rounded-2xl overflow-hidden bg-slate-900 border border-[#1E293B] group"
                            >
                              <img
                              ...
                              />
                              <div className="absolute top-4 left-4 bg-teal-600/90 px-2 py-1 rounded text-[9px] font-bold text-white uppercase">
                                Day {day}
                              </div>
                              <div className="absolute bottom-4 left-4 right-4 bg-black/40 backdrop-blur-sm p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                <p className="text-[9px] font-bold text-white text-center leading-none uppercase tracking-tighter">
                                  {
                                    RECOVERY_TIMELINE.find((s) => day <= s.val)
                                      ?.stage
                                  }
                                </p>
                              </div>
                    ...
                    <motion.div
                      ...
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="absolute inset-0 p-8 flex flex-col bg-slate-950 overflow-y-auto custom-scrollbar"
                    >
                      <div className="flex-1 min-h-[400px] relative rounded-[2rem] overflow-hidden border border-[#1E293B] bg-slate-900">
                        ...
                        <div className="absolute top-8 right-8 flex flex-col items-end gap-2">
                          <span className="px-4 py-2 bg-teal-600 text-white rounded-xl text-xs font-bold shadow-2xl">
                            {
                              RECOVERY_TIMELINE.find((s) => postOpDays <= s.val)
                                ?.stage
                            }
                          </span>
                      ...
                      <div className="mt-8 flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                        {RECOVERY_TIMELINE.map((stage) => (
                          <button
                            ...
                            className={cn(
                              "shrink-0 w-[180px] p-4 rounded-2xl border transition-all text-left",
                              postOpDays === stage.val
                    ...
                    <motion.div
                      ...
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="absolute inset-0 bg-white p-12 overflow-y-auto"
                    >
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest leading-none">
                                Clinical Precision Aesthetic Analysis (V3.0)
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-8">
                          <div className="col-span-2 space-y-8">
                            <section className="space-y-4">
                              <h4 className="flex items-center gap-2 text-xs font-bold text-slate-800 uppercase border-l-4 border-teal-600 pl-3">
                                解剖学深度观察 (Anatomical Insights)
                              </h4>
                              <div className="p-6 bg-slate-50 rounded-[2rem] text-xs text-slate-600 leading-relaxed font-medium italic border border-slate-100">
                                {clinicalAnalysis ? (
                                  <div className="prose prose-slate prose-sm max-w-none">
                                    <Markdown>{clinicalAnalysis}</Markdown>
                                  </div>
                                ) : (
                                  "正在进行三庭五眼深度扫描与骨骼支撑度评估..."
                                )}
                              </div>
                            </section>
                            <section className="space-y-4">
                              <h4 className="flex items-center gap-2 text-xs font-bold text-slate-800 uppercase border-l-4 border-teal-600 pl-3">
                                手术方案执行细节 (Plan Details)
                              </h4>
                              <div className="grid grid-cols-2 gap-3">
                                {Object.entries(intensities)
                                  .filter(([_, v]) => (v as number) > 0)
                                  .map(([k, v]) => {
                                    const it = CATEGORIES.flatMap(
                                      (c) => c.items,
                                    ).find((i) => i.id === k);
                                    return (
                                      <div
                                      ...
                                      >
                                        ...
                                        <span className="px-2 py-0.5 bg-teal-50 text-teal-600 text-[9px] font-bold rounded">
                                          Lv.{v} 强度
                                        </span>
                                      </div>
                                    );
                          ...
                          <div className="space-y-8">
                            ...
                            <section className="p-5 bg-slate-900 rounded-[2rem] text-white space-y-4">
                              ...
                              <ul className="space-y-2">
                                <li className="text-[10px] leading-relaxed opacity-80">
                                  • 注意
                                  {surgeryMethod === "invasive"
                                    ? "切口感染风险"
                                    : "局部神经压迫"}
                                </li>
                                <li className="text-[10px] leading-relaxed opacity-80">
                              ...
                              >
                                <div className="aspect-[3/4] relative">
                                  <img
                                    ...
                                    className="w-full h-full object-cover"
                                  />
                                  <div className="absolute top-2 left-2 bg-black/60 text-white text-[8px] font-bold px-2 py-1 rounded backdrop-blur-sm z-10">
                                    {res.taskName}
                                  </div>
                                ...
                                <div className="p-4">
                                  <div className="flex justify-between items-start mb-1">
                                    <span className="text-[10px] font-black text-slate-800 truncate max-w-[100px]">
                                      {res.taskName}
                                    </span>
                                    <span className="text-[9px] font-bold text-teal-600 bg-teal-50 px-1.5 rounded">
                                      Day {res.settings?.postOpDays}
                                    </span>
                                  </div>
                                  ...
                                  {res.prompt && (
                                    <div className="mt-2 pt-2 border-t border-slate-50">
                                      <p className="text-[8px] text-slate-400 italic line-clamp-1">
                                        "{res.prompt}"
                                      </p>
                                    </div>
                                  )}
                    ...
                    <div className="relative w-40 h-40 mb-8">
                      ...
                      <svg className="w-full h-full -rotate-90">
                        ...
                        <circle
                          ...
                          strokeDashoffset={
                            2 * Math.PI * 74 * (1 - genProgress / 100)
                          }
                          className="transition-all duration-300"
                        />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-4xl font-mono font-bold text-teal-500">
                          {Math.floor(genProgress)}%
                        </span>
                        <span className="text-[10px] font-bold text-teal-500/50 uppercase tracking-widest">
                          Processing
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">
                        术后医学提示 (Post-OP Notice)
                      </span>
                      <p className="text-xs text-slate-600 font-bold leading-relaxed">
                        {postOpDays}天后状态：肿胀已进入
                        {RECOVERY_TIMELINE.find((s) => postOpDays <= s.val)
                          ?.stage || "稳定期"}
                        。
                      </p>
                  ...
                  <div className="flex gap-12">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-500 font-medium italic">
                        {((postOpDays / 30) * 100).toFixed(0)}% 成熟度 /
                        临床参考级
                      </p>
                    </div>
                  </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <h5 className="font-bold text-slate-800 mb-0.5 group-hover:text-teal-600">
                            {p.name}
                          </h5>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none mb-2">
                            Patient Index #{(p.id || "").substring(0, 6)}
                          </p>
                          <div className="flex gap-1.5 flex-wrap">
                            {Object.entries(p.intensities || {})
                              .filter(([_, v]) => (v as number) > 0)
                              .slice(0, 2)
                              .map(([k, v]) => (
                                <span
                                  key={k}
                                  className="px-1.5 py-0.5 bg-slate-100 text-slate-400 text-[8px] font-bold rounded uppercase"
                                >
                                  {k.split("_")[0]}
                                </span>
                              ))}
            ...
            >
              <div className="bg-white px-8 py-4 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">
                      Content Production Engine v3.0
                    </p>
                  </div>
                </div>
                <div className="flex p-1 bg-slate-100 rounded-xl ml-4">
                  ...
                  <button
                    onClick={() => setMainModule("beauty-material")}
                    className={cn(
                      "px-4 py-1.5 rounded-lg text-xs font-bold transition-all",
                      mainModule === "beauty-material"
                ...
                <button
                  onClick={() => setShowVlogLab(false)}
                  className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all"
                >
                  <Plus className="w-6 h-6 rotate-45" />
                </button>
              </div>
                ...
                {mainModule === "vlog" ? (
                      ...
                      <section className="space-y-4">
                        ...
                        <div className="space-y-4 pb-4 border-b border-slate-50">
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-indigo-600 flex items-center gap-1.5 uppercase tracking-wider">
                              <KeyRound className="w-3 h-3" /> OpenAI / 中转站
                              Key
                            </label>
                            <input
                              type="password"
                            ...
                            <div className="space-y-3">
                              <div className="grid grid-cols-3 gap-2">
                                ...
                                >
                                  ...
                                  <span className="text-[9px] font-black text-slate-500 group-hover:text-emerald-700">
                                    选题推荐
                                  </span>
                                </button>
                                <button
                                  onClick={() =>
                                    handleGenerateVlog("storyboard")
                                  }
                                  disabled={isGeneratingVlog}
                                ...
                                >
                                  ...
                                  <span className="text-[9px] font-black text-slate-500 group-hover:text-amber-700">
                                    云端分镜
                                  </span>
                                </button>
                                <button
                                ...
                                >
                                  ...
                                  <span className="text-[9px] font-black text-slate-500 group-hover:text-slate-700">
                                    智能标签
                                  </span>
                                </button>
                              </div>
                          ...
                          <div className="pt-4 border-t border-slate-50">
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl">
                                ...
                                <button
                                  onClick={() =>
                                    setVlogCharacterConsistency(
                                      !vlogCharacterConsistency,
                                    )
                                  }
                                  className={cn(
                                    "w-8 h-4 rounded-full transition-all relative",
                              ...
                              {false && (
                                <div className="relative group aspect-square rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex flex-col items-center justify-center transition-all hover:border-indigo-300 hover:bg-slate-100">
                                  {vlogReferenceImage ? (
                                    <React.Fragment>
                                      <img
                                        src={vlogReferenceImage}
                                        className="w-full h-full object-cover"
                                      />
                                      <button
                                        onClick={() =>
                                          setVlogReferenceImage(null)
                                        }
                                    ...
                                    <label className="cursor-pointer w-full h-full flex flex-col items-center justify-center p-4">
                                      ...
                                      <span className="text-[10px] font-bold text-slate-400 text-center">
                                        点击上传主角照片
                                        <br />
                                        <small className="font-normal opacity-50">
                                          锁定人物五官特征
                                        </small>
                                      </span>
                                    </label>
                        ...
                        <div className="space-y-4">
                          <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1.5">
                            <Zap className="w-3 h-3" /> 图片引擎 (Image Engine)
                          </label>
                            ...
                            {(["stock", "dalle"] as const).map((engine) => (
                              ...
                              >
                                {engine === "stock"
                                  ...
                                  : "GPT DALL-E"}
                                {engine === "dalle" && (
                                  <div className="absolute top-0 right-0 w-3 h-3 bg-amber-500 text-white flex items-center justify-center scale-50 rounded-bl-lg">
                                    ★
                                  </div>
                                )}
                              </button>
                        ...
                        <div className="pt-2">
                          ...
                          >
                            ...
                            <div
                              className={cn(
                                "w-8 h-4 rounded-full transition-all relative transition-colors",
                                vlogVars.chineseStyle
                                  ? "bg-amber-400"
                                  : "bg-slate-300",
                              )}
                            ...
                            <div className="w-32 h-32 relative mb-8">
                              ...
                              <div className="absolute inset-0 flex items-center justify-center">
                                <span className="text-white font-black text-xl">
                                  {Math.round(vlogGenProgress)}%
                                </span>
                              </div>
                            </div>
                            <div className="space-y-3 text-center">
                              <p className="text-white font-black text-lg tracking-widest uppercase">
                                {vlogGenStatus || "AI Engine Initializing..."}
                              </p>
                              <p className="text-indigo-400 font-bold text-[10px] tracking-[0.2em] uppercase opacity-50">
                                Generative Marketing Pipeline v2.0
                              </p>
                            </div>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                  {vlogResult.topics
                                    ...
                                    .map((t, i) => (
                                      <div
                                        key={`topic-${i}`}
                                        className="group p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all flex items-center justify-between"
                                      >
                                        ...
                                        <button
                                          ...
                                          className="p-2 opacity-0 group-hover:opacity-100 bg-indigo-600 rounded-lg text-white transition-opacity"
                                        >
                            ...
                            {activeVlogTab === "storyboard" && (
                              <section className="space-y-8">
                                <h5 className="flex items-center gap-3 text-amber-400 font-bold text-xs uppercase tracking-widest">
                                  <Video className="w-4 h-4" /> 片段拼接分镜表
                                  (Storyboard Timeline)
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  {vlogResult.storyboard
                                    ?.filter((c) => c && typeof c === "object")
                                    .map((clip, idx) => (
                                      <div
                                        key={`clip-${idx}-${clip.Scene || idx}`}
                                        className="bg-white/5 border border-white/10 rounded-2xl p-5 group flex gap-4"
                                      >
                                      <div className="w-24 h-32 bg-slate-800 rounded-lg flex-shrink-0 relative overflow-hidden border border-white/5 shadow-2xl">
                                        ...
                                        <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-black/60 rounded text-[9px] font-bold text-white tracking-widest backdrop-blur-sm">
                                          #{idx + 1}
                                        </div>
                                        <button
                                          onClick={(e) => {
                                      ...
                                      <div className="flex-1 flex flex-col justify-between py-1">
                                        <div>
                                          <h6 className="text-amber-400 text-[11px] font-black uppercase tracking-widest mb-1">
                                            {clip.Scene}
                                          </h6>
                                          <p className="text-white/60 text-[10px] leading-relaxed line-clamp-3 italic">
                                            "{clip["Imagen Prompt"]}"
                                          </p>
                                        </div>
                                        <button
                                ...
                                <div className="flex gap-3 flex-wrap">
                                  {vlogResult.tags
                                    ...
                                    .map((tag, idx) => (
                                      <span
                                        key={`tag-${idx}-${tag}`}
                                        className="px-5 py-2.5 bg-white/5 text-white/80 text-[11px] font-bold rounded-2xl border border-white/10 hover:bg-white/10 transition-all cursor-default"
                                      >
                                        {tag}
                                      </span>
                                    ))}
                    ...
                    <div className="w-80 border-r border-slate-100 flex flex-col h-full bg-white overflow-hidden shadow-sm">
                      <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setBatchImageTaskName(e.target.value)
                              }
                          ...
                          <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                              <Layout className="w-2.5 h-2.5" /> 构图比例
                              (Aspect Ratio)
                            </label>
                            <div className="grid grid-cols-3 gap-2">
                              {(["1:1", "9:16", "16:9"] as const).map(
                                (ratio) => (
                                  <button
                                    key={ratio}
                                    onClick={() =>
                                      setBatchImageAspectRatio(ratio)
                                    }
                                    className={cn(
                          ...
                          <div className="space-y-3">
                            <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1.5">
                              <Sparkles className="w-3 h-3" /> 输入 10
                              个独立提示词
                            </label>
                              ...
                              {batchImagePrompts.map((p, idx) => (
                                <div key={idx} className="space-y-1.5 relative">
                                  <div className="flex justify-between items-center group">
                                    ...
                                    {batchImageProgress[idx] === 100 && (
                                      <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                                    )}
                                    {batchImageProgress[idx] === -1 && (
                                  ...
                                  <textarea
                                    ...
                                    className={cn(
                                      ...
                                      batchImageProgress[idx] > 0 &&
                                        batchImageProgress[idx] < 100 &&
                                        "ring-2 ring-indigo-500/10 animate-pulse border-indigo-200",
                                    )}
                                    ...
                                    batchImageProgress[idx] < 100 && (
                                      <div className="absolute bottom-2 right-2 flex items-center gap-1.5 bg-white/80 backdrop-blur-sm px-1.5 py-0.5 rounded-md border border-slate-100 shadow-sm">
                                        ...
                                        <span className="text-[8px] font-bold text-indigo-500">
                                          {Math.floor(batchImageProgress[idx])}%
                                        </span>
                                      </div>
                                    )}
                                </div>
                        ...
                        <div className="pt-4 border-t border-slate-100 space-y-4">
                          ...
                          >
                            <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1.5">
                              <Server className="w-3 h-3 text-indigo-600" /> API
                              并行通道配置
                            </span>
                            <ChevronRight
                              className={cn(
                            ...
                            <div className="p-3 bg-slate-50/50 rounded-xl border border-slate-100 space-y-3 animate-in slide-in-from-top-2">
                              <div className="space-y-1">
                                <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider">
                                  Base URL (OpenAI Proxy)
                                </label>
                                <input
                                  value={faceSwapBaseUrl}
                                  onChange={(e) =>
                                    setFaceSwapBaseUrl(e.target.value)
                                  }
                              ...
                              <div className="space-y-1">
                                ...
                                <input
                                  ...
                                  value={faceSwapApiKey}
                                  onChange={(e) =>
                                    setFaceSwapApiKey(e.target.value)
                                  }
                                  className="w-full p-2 text-[10px] bg-white border border-slate-200 rounded-lg outline-none focus:border-indigo-500"
                                />
                              </div>
                              <div className="space-y-1">
                                ...
                                <input
                                  value={faceSwapImageModel}
                                  onChange={(e) =>
                                    setFaceSwapImageModel(e.target.value)
                                  }
                                  className="w-full p-2 text-[10px] bg-white border border-slate-200 rounded-lg outline-none focus:border-indigo-500"
                        ...
                        <div className="pt-8">
                          <button
                            disabled={
                              isGeneratingBatchImage ||
                              !batchImagePrompts.some((p) => p.trim() !== "")
                            }
                        ...
                        {selectedBatchImg ? (
                            ...
                            <div className="absolute bottom-8 left-8 right-8">
                              <div className="bg-black/40 backdrop-blur-md rounded-2xl p-4 border border-white/10">
                                <p className="text-white/80 text-[10px] leading-relaxed italic text-center">
                                  "
                                  {
                                    batchImageResults.find(
                                      (r) => r.url === selectedBatchImg,
                                    )?.prompt
                                  }
                                  "
                                </p>
                          ...
                          <div className="text-center opacity-20 select-none animate-pulse">
                            ...
                            <p className="text-slate-400 text-[10px] mt-2 font-bold uppercase tracking-widest">
                              Select an item from the library below
                            </p>
                          </div>
                        )}
                      </div>
                        ...
                        <div className="h-44 bg-black/80 backdrop-blur-3xl border-t border-white/10 p-6 flex flex-col gap-4 overflow-hidden relative">
                          <div className="flex items-center justify-between px-3">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-400 text-[8px] font-bold rounded-full">
                                {batchImageResults.length} Units
                              </span>
                            </div>
                            <button
                              disabled={downloadingZip || batchImageResults.length === 0}
                              onClick={() => {
                                downloadBatchAsZip(
                                  batchImageResults,
                                  `batch_gen_${batchImageTaskName || Date.now()}.zip`
                                );
                              }}
                              className={cn(
                                "flex items-center gap-2 group transition-all",
                                (downloadingZip || batchImageResults.length === 0) ? "opacity-50 cursor-not-allowed" : "hover:opacity-80"
                              )}
                            >
                              <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest group-hover:text-indigo-300 transition-colors">
                                {downloadingZip ? "Processing ZIP..." : "Download All as ZIP"}
                              </span>
                              {downloadingZip ? (
                            ...
                            <button
                              onClick={() => {
                                if (confirm("确定要清空批量生成的所有历史图片吗？")) {
                                  setBatchImageResults([]);
                                  setSelectedBatchImg(null);
                                }
                              }}
                              className="flex items-center gap-2 group"
                            >
                              <span className="text-[9px] font-red text-rose-400 uppercase tracking-widest group-hover:text-rose-300 transition-colors">
                                Clear History
                              </span>
                            ...
                            {batchImageResults.map((res) => (
                              <button
                                ...
                                className={cn(
                                  ...
                                  selectedBatchImg === res.url
                                    ? "border-indigo-500 scale-[1.05] z-10 shadow-indigo-500/20"
                                    : "border-white/10 hover:border-white/30 grayscale-20 hover:grayscale-0",
                                )}
                              >
                                <img
                                  src={res.url}
                  ...
                  <div className="flex-1 flex flex-row overflow-hidden bg-white animate-in fade-in duration-500">
                    ...
                    <div className="w-80 border-r border-slate-100 flex flex-col h-full bg-white overflow-hidden shadow-sm">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setFaceSwapTaskName(
                                    `医美素材_${t.name}_${new Date().toLocaleTimeString("zh-CN", { hour12: false })}`,
                                  );
                                  alert(`已切换至模板: ${t.name}`);
                                }}
                                className="flex flex-col items-center gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:border-rose-200 hover:bg-rose-50/30 transition-all group"
                              >
                                <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-slate-400 group-hover:text-rose-500 shadow-sm transition-colors">
                                  {t.icon}
                                </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            {[
                              {
                                name: "妈生感自然微调",
                                desc: "强调皮肤纹理，光影柔和，去人工智能感",
                                prompt:
                                  'Natural skin texture, soft lighting, subtle enhancements, "I woke up like this" look.',
                              },
                              {
                                name: "高清镜头幼态脸",
                                desc: "饱满额头，幼态比例，皮肤清透感",
                                prompt:
                                  "Bright eyes, full cheeks, smooth jawline, porcelain skin, youthful radiance.",
                              },
                              {
                                name: "极简冷风分析图",
                                desc: "冷色调背景，专业光影，轮廓深邃",
                                prompt:
                                  "Cool-toned grading, sharp contours, modern clinical elegance.",
                              },
                              {
                                name: "中式审美侧颜杀",
                                desc: "骨相立体，中式皮相，温婉高级",
                                prompt:
                                  "Defined features, dramatic lighting, luxury fashion aesthetic, impeccable skin.",
                              },
                            ].map((p) => (
                              <button
                                key={p.name}
                                onClick={() => setFaceSwapPrompt(p.prompt)}
                                className="w-full p-4 text-left bg-white border border-slate-100 rounded-2xl hover:shadow-xl hover:scale-[1.02] transition-all group relative overflow-hidden"
                              >
                                <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <Plus className="w-3 h-3 text-rose-400" />
                                </div>
                                <div className="text-[11px] font-bold text-slate-800 group-hover:text-rose-600 mb-1">
                                  {p.name}
                                </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="w-8 h-4 bg-slate-200 rounded-full cursor-pointer" />
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-bold text-slate-500">
                                日期水印
                              </span>
                              <div className="w-8 h-4 bg-rose-500 rounded-full cursor-pointer relative">
                      ...
                      <div className="p-6 border-t border-slate-100">
                        <button
                          onClick={handleBeautyGen}
                          disabled={isGeneratingVlog}
                          className="w-full py-4 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-rose-100 transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-50"
                        >
                          {isGeneratingVlog ? (
                            <RotateCcw className="w-4 h-4 animate-spin" />
                          ) : (
                    ...
                    <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden relative">
                      <div className="flex-1 p-10 flex items-center justify-center relative overflow-hidden">
                        <div className="w-full max-w-5xl grid grid-cols-2 gap-8 h-full items-center">
                          {/* Before Card */}
                          <div
                            onClick={() => {
                              ...
                              input.onchange = (e: any) => {
                                ...
                                if (file) {
                                  const reader = new FileReader();
                                  reader.onload = (re) =>
                                    setBeautyUploads((prev) => ({
                                      ...prev,
                                      before: re.target?.result as string,
                                    }));
                                  reader.readAsDataURL(file);
                                }
                              };
                            ...
                            className="aspect-[9/16] bg-white rounded-[2.5rem] shadow-2xl border border-white relative overflow-hidden flex flex-col group cursor-pointer"
                          >
                            <div className="absolute top-6 left-6 z-10 px-4 py-2 bg-black/30 backdrop-blur-md rounded-full border border-white/20">
                              <span className="text-[10px] font-black text-white uppercase tracking-widest">
                                Before
                              </span>
                            </div>
                            <div className="flex-1 flex items-center justify-center bg-slate-50 border-b border-slate-100">
                              {beautyUploads?.before ? (
                                <img
                                  src={beautyUploads.before}
                                  className="w-full h-full object-cover"
                                />
                              ) : (
                          ...
                          <div
                              ...
                              input.onchange = (e: any) => {
                                ...
                                if (file) {
                                  ...
                                  reader.onload = (re) =>
                                    setBeautyUploads((prev) => ({
                                      ...prev,
                                      after: re.target?.result as string,
                                    }));
                                  reader.readAsDataURL(file);
                                }
                              };
                            ...
                            className="aspect-[9/16] bg-white rounded-[2.5rem] shadow-2xl border border-white relative overflow-hidden flex flex-col group cursor-pointer"
                          >
                            <div className="absolute top-6 left-6 z-10 px-4 py-2 bg-rose-500 rounded-full shadow-lg shadow-rose-200">
                              <span className="text-[10px] font-black text-white uppercase tracking-widest">
                                After
                              </span>
                            </div>
                        ...
                        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-3 p-2 bg-white/80 backdrop-blur-2xl rounded-3xl border border-white shadow-2xl shadow-slate-200/50 scale-110">
                          <button className="p-4 bg-slate-100 text-slate-400 rounded-2xl hover:bg-slate-200 transition-colors">
                            <Plus className="w-5 h-5" />
                          </button>
                          <button className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-xs uppercase tracking-tighter shadow-lg">
                            合成素材库
                          </button>
                          <button className="p-4 bg-rose-500 text-white rounded-2xl hover:bg-rose-600 transition-colors shadow-lg shadow-rose-200">
                    ...
                    <div className="w-80 border-r border-slate-100 flex flex-col h-full bg-white overflow-hidden shadow-sm">
                      <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1.5">
                              <Sparkles className="w-3 h-3" /> 合成增强提示词
                              (Prompt)
                            </label>
                            <textarea
                              value={faceSwapPrompt}
                              onChange={(e) =>
                                setFaceSwapPrompt(e.target.value)
                              }
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1.5">
                                <Server className="w-3 h-3" /> 并行 API 通道配置
                                ({faceSwapApis.filter((a) => a).length}/10)
                              </span>
                              <ChevronRight
                                className={cn(
                                  "w-3 h-3 text-slate-300 transition-transform",
                            ...
                            {showApiConfig && (
                              <div className="space-y-4 p-3 bg-slate-50/50 rounded-xl border border-slate-100 animate-in slide-in-from-top-2 duration-300">
                                <div className="space-y-2">
                                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                    <Globe className="w-2.5 h-2.5" /> API 中转站
                                    Base URL
                                  </label>
                                  <input
                                    value={faceSwapBaseUrl}
                                ...
                                <div className="space-y-2 border-b border-slate-50 pb-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setFaceSwapTaskName(e.target.value)
                                    }
                                ...
                                <div className="space-y-2 mt-4 pt-2 border-t border-slate-100">
                                  <button
                                    onClick={() => {
                                      if (confirm("确定要清空所有本地结果库吗？此操作不可撤销，且将刷新页面。")) {
                                        localStorage.clear();
                                        window.location.reload();
                                      }
                                    }}
                                    className="w-full py-2 bg-rose-50 text-rose-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-rose-100 hover:bg-rose-100 transition-all"
                                  >
                                    清理本地缓存 (Reset Data)
                                  </button>
                                </div>
                                ...
                                <div className="space-y-2 mt-2">
                                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                    <Monitor className="w-2.5 h-2.5" /> 换脸模型
                                    (Model)
                                  </label>
                                  <input
                                    value={faceSwapModel}
                                    onChange={(e) =>
                                      setFaceSwapModel(e.target.value)
                                    }
                                    className="w-full p-2 text-[10px] bg-white border border-slate-200 rounded-lg outline-none focus:border-indigo-500"
                                    placeholder="如: gpt-4o 或保持为空"
                                  />
                                  <p className="text-[8px] text-slate-400">
                                    * 针对灯塔 (Dengche) API，建议使用
                                    gpt-image-2 模型。
                                  </p>
                                </div>
                          ...
                          <div className="space-y-4">
                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                              ...
                              基准人脸 (Source Identity)
                            </h4>
                            <div
                              onClick={() =>
                                document
                                  .getElementById("source-upload")
                                  ?.click()
                              }
                              className={cn(
                            ...
                            >
                              {faceSwapInputs.source ? (
                                <>
                                  <img
                                    src={faceSwapInputs.source}
                                    className="w-full h-full object-cover"
                                  />
                                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <span className="text-[10px] font-black text-white uppercase tracking-widest px-4 py-2 bg-white/20 backdrop-blur-md rounded-lg">
                                      Replace Source
                                    </span>
                                  </div>
                                </>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-5 gap-1.5">
                              {faceSwapInputs.targets.map((url, idx) => (
                                <div
                                  key={`target-${idx}`}
                                  className="relative aspect-square rounded-lg bg-slate-50 overflow-hidden border border-slate-100 group"
                                >
                                  <img
                                    src={url}
                                    className="w-full h-full object-cover"
                                  />
                                  {faceSwapProgress[idx] !== undefined &&
                                      ...
                                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center p-1">
                                        <div className="w-full h-full rounded-full border-2 border-indigo-500/20 flex items-center justify-center relative">
                                          ...
                                          <span className="text-[8px] font-black text-white">
                                            {Math.floor(faceSwapProgress[idx])}%
                                          </span>
                                        </div>
                                      </div>
                                    )}
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const remaining =
                                  10 - faceSwapInputs.targets.length;
                    ...
                    <div className="flex-1 flex flex-col bg-slate-900 border-x border-slate-100 overflow-hidden relative">
                        ...
                        {selectedFaceImg ? (
                          ...
                          >
                            ...
                            <div className="absolute top-8 right-8">
                              <button
                                onClick={() =>
                                  downloadImage(selectedFaceImg!, "hq-render")
                                }
                                className="p-5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-3xl shadow-xl transition-all active:scale-[0.85] shadow-indigo-500/20"
                              >
                      ...
                      {faceSwapResults.length > 0 && (
                        <div className="h-44 bg-black/80 backdrop-blur-3xl border-t border-white/10 p-6 flex flex-col gap-4 overflow-hidden relative">
                          <div className="flex items-center justify-between px-3">
                            <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest">
                              Synthesis Library ({faceSwapResults.length})
                            </h5>
                             <div className="flex items-center gap-3">
                               <button
                                 disabled={downloadingZip || faceSwapResults.length === 0}
                                 onClick={() => {
                                   downloadBatchAsZip(
                                     faceSwapResults,
                                     `faceswap_gen_${faceSwapTaskName || Date.now()}.zip`
                                   );
                                 }}
                                 className={cn(
                                   "flex items-center gap-1.5 group transition-all",
                                   (downloadingZip || faceSwapResults.length === 0) ? "opacity-50 cursor-not-allowed" : "hover:opacity-80"
                                 )}
                               >
                                 <span className="text-[8px] font-black text-indigo-400/80 uppercase tracking-widest group-hover:text-indigo-400 transition-colors">
                                   {downloadingZip ? "Zipping..." : "ZIP All"}
                                 </span>
                                 <Download className={cn("w-2.5 h-2.5 text-indigo-400/80", downloadingZip && "animate-bounce")} />
                               </button>
                               <button
                                 onClick={() => {
                                   if (confirm("确定要清空换脸生成的所有历史图片吗？")) {
                                     setFaceSwapResults([]);
                                     setSelectedFaceImg(null);
                                   }
                                 }}
                                 className="flex items-center gap-1.5 group"
                               >
                                 <span className="text-[8px] font-black text-rose-500/50 uppercase tracking-widest group-hover:text-rose-500 transition-colors">
                                   Clear Results
                                 </span>
                                 <Trash2 className="w-2.5 h-2.5 text-rose-500/50 group-hover:text-rose-500" />
                               </button>
                             </div>
                          </div>
                          <div className="flex gap-4 overflow-x-auto custom-scrollbar pb-3 px-3">
                            {faceSwapResults.map((res) => (
                              <button
                                ...
                                className={cn(
                                  "flex-shrink-0 h-24 aspect-[3/4] rounded-2xl overflow-hidden border-2 transition-all relative group shadow-lg",
                                  selectedFaceImg === res.url
                                    ? "border-indigo-500 scale-[1.05] z-10"
                                    : "border-white/10 hover:border-white/30 grayscale-20",
                                )}
                              >
                                <img
                                  src={res.url}
                            ...
                            <div key={tool.key} className="space-y-3">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFaceFilters((prev) => ({
                                    ...prev,
                                    [tool.key]: parseInt(e.target.value),
                                  }))
                                }
                                className="w-full"
                              />
      ...
      <footer className="fixed bottom-0 left-[340px] right-0 bg-white/80 backdrop-blur-sm border-t border-slate-100 px-6 py-2 z-40">
        <p className="text-[9px] text-slate-400 text-center font-medium">
          模拟结果基于 AI
          算法生成，仅供临床沟通参考。实际效果受个体差异、手术入路及恢复环境影响。请咨询具备执业资质的整形外科医生。
        </p>
      </footer>
